 ...
 *    Use sql/install.sql with 0_ table prefix.
 ...
 * 
 * @package KsfFA_ksf_FA_Tracking
 * @version 2.4.3-0
 */

define('SS_TRACKING', 139 << 8);

class hooks_ksf_FA_Tracking extends hooks {
    var $module_name = 'ksf_FA_Tracking';
    var $version = '2.4.3-0';
    ...
    function install_access() {
        $security_sections[SS_TRACKING] = _("Tracking");
        $security_areas['SA_TRACKING_VIEW'] = array(
            SS_TRACKING | 1, 
            _("View ")
        );
        $security_areas['SA_TRACKING_MANAGE'] = array(
            SS_TRACKING | 2, 
            _("Manage ")
        );
    ...
    function activate_extension($company, $check_only=true) {
        ...
        // This handles 0_ table prefix replacement automatically
    ...
    private function ensure_composer_dependencies() {
        ...
        chdir($prev_dir);
        if ($return_code !== 0) {
            error_log('ksf_FA_Tracking: composer install failed: ' . implode("\n", $output));
        }
    }
}
